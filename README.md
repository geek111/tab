...
Starting with version 0.4 the extension introduces experimental tab groups. Tabs can be assigned to named groups via the tab context menu and groups are saved between sessions. Groups can also be managed from the Options page. Version 0.4.2 adds a dynamic "Add Tab(s) to Group" menu that lists your groups and allows creating a new one on the fly. Version 0.4.3 provides a basic Groups view in the popup and full pages for bulk operations on each group. Version 0.4.4 adds a dropdown and button for assigning selected tabs to a group directly from the popup or full view.
