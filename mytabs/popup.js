...
async function bulkAddToGroup(groupId) {
  ...
  try {
    const firstTab = await browser.tabs.get(ids[0]);
    const createProperties = { windowId: firstTab.windowId, index: firstTab.index };
    await browser.tabs.group({ groupId, tabIds: ids, createProperties });
  } catch (e) {
