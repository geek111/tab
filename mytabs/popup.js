...
async function loadGroups() {
  try {
    ...
    tabGroupMap = new Map(tabs.map(t => [t.id, t.groupId]));
  } catch (e) {
...
function renderTabs(list, activeId, dupIds, visitedIds, winMap, query = '') {
  ...
  const groupNames = new Map(groups.map(g => [g.id, g.title || `Group ${g.id}`]));
...
function showContextMenu(e) {
  ...
  if (selected.length) {
    addItem('Remove from Group', () => assignSelectedToGroup(null));
    for (const g of groups) {
      const label = g.title || `Group ${g.id}`;
      addItem(`Assign to ${label}`, () => assignSelectedToGroup(g.id));
    }
  }

  addItem('Create Group', createGroupPrompt);
  for (const g of groups) {
    const label = g.title || `Group ${g.id}`;
    addItem(`Delete Group ${label}`, () => deleteGroup(g.id));
  }
...
async function createGroupPrompt() {
  ...
  const ids = getSelectedTabIds();
  if (!ids.length) return;
  await browser.tabs.group({ tabIds: ids, createProperties: { title: name } });
  await loadGroups();
  scheduleUpdate();
}
