...
async function loadGroups() {
  ...
  if (target) {
    ...
    target.value = prev && groups.some(g => String(g.id) === prev) ? prev : (groups[0]?.id || '');
    target.disabled = groups.length === 0;
    const addBtn = document.getElementById('bulk-add-group');
    if (addBtn) addBtn.disabled = groups.length === 0;
  }
}
...
async function init() {
  ...
  const groupTargetSel = document.getElementById('group-target');
  if (addGroupBtn && groupTargetSel) {
    addGroupBtn.addEventListener('click', () => {
      const id = groupTargetSel.value;
...
function showContextMenu(e) {
  ...
  if (selected.length) {
    addSubMenu('Group', menu => {
      addItem('Remove from Group', () => assignSelectedToGroup(null), menu);
      for (const g of groups) {
        addItem(`Assign to ${g.name}`, () => assignSelectedToGroup(g.id), menu);
      }
