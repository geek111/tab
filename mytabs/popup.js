...
function showContextMenu(e) {
  ...
  if (selected.length) {
    ...
    for (const g of groups) {
      addItem(`Assign to ${g.name}`, () => assignSelectedToGroup(g.id));
    }
  }

  addItem('Create Group', createGroupPrompt);
  for (const g of groups) {
    addItem(`Delete Group ${g.name}`, () => deleteGroup(g.id));
  }
...
async function createGroupPrompt() {
  ...
  if (!name || groups.some(g => g.name === name.trim())) {
    alert('Invalid or duplicate name');
    return;
  }
  await browser.runtime.sendMessage({ type: 'createGroup', name: name.trim() });
  await loadGroups();
  scheduleUpdate();
}
...
async function deleteGroup(id) {
  ...
  await browser.runtime.sendMessage({ type: 'deleteGroup', id });
  await loadGroups();
  scheduleUpdate();
}
