...
async function update() {
  ...
  try {
    ...
    const queryOpts = allWins ? { windowType: 'normal' } : { currentWindow: true };
    ...
    let activeCount;
    if (allWins) {
      let activeTabs = await browser.tabs.query({ active: true, windowType: 'normal' });
      if (filterContainerId) {
        activeTabs = activeTabs.filter(t => t.cookieStoreId === filterContainerId);
      }
      activeCount = activeTabs.length;
    } else {
      activeCount = allTabs.filter(t => t.active).length;
    }
