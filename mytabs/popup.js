...
function showContextMenu(e) {
  ...
  if (selected.length) {
    ...
    if (kepiGroups && kepiGroups.groups.length) {
      kepiGroups.groups.forEach(g => {
        addItem(`Add to ${g.name}`, () => {
          selected.forEach(id => kepiGroups.assignTabToGroup(id, g.id));
          scheduleUpdate();
        });
      });
    }
    addItem('Add to New Group…', async () => {
      const name = prompt('Group name:');
      if (name) {
        const group = kepiGroups.createGroup(name);
        selected.forEach(id => kepiGroups.assignTabToGroup(id, group.id));
        scheduleUpdate();
      }
    });
  }
