...
browser.contextMenus.onClicked.addListener((info, tab) => {
  if (info.menuItemId === 'open-options') {
  ...
  } else if (info.menuItemId === 'create-group') {
    const name = prompt('New group name:');
    if (name) {
      kepiGroups.createGroup(name);
    }
  } else if (info.menuItemId === 'delete-group') {
    const name = prompt('Delete which group?');
    const group = kepiGroups.groups.find(g => g.name === name);
    if (group && confirm(`Delete group "${name}"?`)) {
      kepiGroups.removeGroup(group.id);
    }
    ...
    const name = prompt('Add to group (name):');
    if (name) {
