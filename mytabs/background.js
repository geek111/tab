...
async function confirmTabDiscarded(tabId, attempts = 10, waitMs = 150) {
...
async function ensureTabCanBeDiscarded(tabId) {
  try {
    const tab = await browser.tabs.get(tabId);
...
async function unloadTabById(tabId) {
  const autoDiscardableChanged = await ensureTabCanBeDiscarded(tabId);
