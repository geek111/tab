...
browser.runtime.onMessage.addListener((msg) => {
  if (msg && msg.type === 'getRecent') {
    ...
    if (!name) {
      return Promise.resolve({ groups, error: 'invalidName' });
    }
    if (groups.some(g => g.name.toLowerCase() === name.toLowerCase())) {
      return Promise.resolve({ groups, error: 'duplicate' });
    }
    ...
    for (const [k, v] of Array.from(tabGroups)) {
      if (v === msg.id) {
        tabGroups.delete(parseInt(k, 10));
        removed++;
      }
    }
    scheduleGroupsSave();
    scheduleTabGroupsSave();
    return Promise.resolve({ groups, removed });
  } else if (msg && msg.type === 'renameGroup') {
    const name = (msg.name || '').trim();
    if (!name) {
      return Promise.resolve({ groups, error: 'invalidName' });
    }
    if (groups.some(g => g.id !== msg.id && g.name.toLowerCase() === name.toLowerCase())) {
      return Promise.resolve({ groups, error: 'duplicate' });
    }
